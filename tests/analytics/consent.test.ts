...
describe("consent cookies", () => {
  it("parses the current version and an allowlisted choice", () => {
    expect(parseConsentCookie("3:analytics")).toEqual({ version: "3", choice: "analytics" })
  })

  it("invalidates pre-disclosure v2 choices and treats malformed values as unknown", () => {
    expect(parseConsentCookie("0:analytics")).toBeNull()
    expect(parseConsentCookie("1:analytics")).toBeNull()
    expect(parseConsentCookie("2:analytics")).toBeNull()
    expect(parseConsentCookie("3:unknown")).toBeNull()
    expect(parseConsentCookie("garbage")).toBeNull()
  })

  it("serializes choices with the current policy version", () => {
    expect(consentCookieValue("essential")).toBe("3:essential")
    expect(consentCookieValue("analytics")).toBe("3:analytics")
  })

  it("exports the required cookie names and secure options", () => {
    expect(CONSENT_POLICY_VERSION).toBe("3")
